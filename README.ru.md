...
В первой версии библиотеки тестируемое приложение предполагается запускать в том же JVM через `@SpringBootTest`.
...
- тестируемое Spring Boot приложение в первой версии предполагается запускать в том же JVM через `@SpringBootTest`.
